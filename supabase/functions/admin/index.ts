...
// Mirrors the `product_category` enum in the database (kept in sync with the `categories` table)
const PRODUCT_CATEGORY_VALUES = ['engines', 'cars', 'flowers', 'other', 'architektura-ir-gelezinkeliai', 'ateities-automobiliai', 'automobiliai', 'aviacijos-varikliai', 'botanika', 'botanika-ir-namu-dekoras', 'buldozeriai-ir-krautuvai', 'dinozauru-pasaulis', 'doko-scenos', 'dyzeliniai-varikliai', 'ekskavatoriai', 'gamtos-kuriniai', 'garveziai', 'gbc-serija', 'izymus-pastatai', 'jachtos-ir-buriavimas', 'karine-technika', 'karyba', 'klasikiniai-ir-retro', 'kosmosas-ir-inzinerija', 'kosmoso-tyrinejimai', 'kranai', 'kurybines-muzikines-dezutes', 'laivai-butelyje', 'laivai-ir-piratai', 'mechaniniai-laikrodziai', 'mechos-kariai', 'miesto-gelbejimo-ir-policijos-technika', 'mini-124-automobiliai', 'mini-botanika', 'mini-dinozaurai', 'mini-garsus-automobiliai', 'mini-gatviu-vaizdai', 'mini-karine-aviacijos-technika', 'mini-miesto-dekoras', 'mini-pakeles-automobiliai', 'mini-pasaulis', 'mini-statybine-technika', 'mini-sunkioji-technika', 'mini-sunkvezimiai', 'mini-visureigiai-modeliai', 'mini-zemes-ukio-technika', 'miskininkystes-technika', 'modernus-ir-greitieji-traukiniai', 'modernus-laivynas', 'moduliniai-pastatai', 'modulinis-gatves-vaizdas', 'motociklai', 'muge-ir-parkas', 'pagal-kebulo-tipa', 'piratu-ir-kariniai-laivai', 'piratu-laivai', 'rc-ir-di-robotai', 'robotai-ir-mechos', 'sakiniai-krautuvai', 'senoves-kariniai-laivai', 'sportiniu-automobiliu-varikliai', 'statyba', 'statyba-ir-sunkioji-technika', 'sunkiasvoriai-sunkvezimiai', 'superautomobiliai-lenktyniniai', 'tankai-ir-sarvuociai', 'technikos-prietaisai', 'technikos-transportas', 'traktoriai-ir-kombainai', 'transportas-ir-logistika', 'traukiniai-ir-gelezinkeliai', 'uostas-ir-pajuris', 'varikliai', 'varikliai-ir-transmisijos', 'viduramziu-ir-retro-pastatai', 'vilkikai-ir-pagalbos-technika', 'visureigiai-ir-suv', 'zaislai-ginklai', 'zemes-ukis-ir-miskininkyste'] as const;
...
const adminRequestSchema = z.discriminatedUnion('action', [
  ...
  z.object({ action: z.literal('create_product'), sku: z.string().min(1), slug: z.string().min(1), title: z.string().min(1), short_desc: z.string().nullable().optional(), description: z.string().nullable().optional(), price_eur: z.number().positive(), deposit_eur: z.number().min(0), sale_price_eur: z.number().nullable().optional(), cost_price_eur: z.number().nullable().optional(), credits_cost_eur: z.number().nullable().optional(), stock_status: z.enum(['preorder', 'in_stock', 'out_of_stock']), status: z.enum(['active', 'inactive']), category: z.enum(PRODUCT_CATEGORY_VALUES), category_id: z.string().uuid().nullable().optional(), images: z.array(z.string()).optional(), badges: z.array(z.string()).optional(), tags: z.array(z.string()).optional(), preorder_eta_weeks_min: z.number().nullable().optional(), preorder_eta_weeks_max: z.number().nullable().optional(), inventory_qty: z.number().optional(), details_json: z.record(z.unknown()).nullable().optional() }),
  z.object({ action: z.literal('update_product'), productId: z.string().uuid(), sku: z.string().min(1).optional(), slug: z.string().min(1).optional(), title: z.string().min(1).optional(), short_desc: z.string().nullable().optional(), description: z.string().nullable().optional(), price_eur: z.number().positive().optional(), deposit_eur: z.number().min(0).optional(), sale_price_eur: z.number().nullable().optional(), cost_price_eur: z.number().nullable().optional(), credits_cost_eur: z.number().nullable().optional(), stock_status: z.enum(['preorder', 'in_stock', 'out_of_stock']).optional(), status: z.enum(['active', 'inactive']).optional(), category: z.enum(PRODUCT_CATEGORY_VALUES).optional(), category_id: z.string().uuid().nullable().optional(), images: z.array(z.string()).optional(), badges: z.array(z.string()).optional(), tags: z.array(z.string()).optional(), preorder_eta_weeks_min: z.number().nullable().optional(), preorder_eta_weeks_max: z.number().nullable().optional(), inventory_qty: z.number().optional(), details_json: z.record(z.unknown()).nullable().optional() }),
