...
import { 
  searchTerminals, 
  getCarrierFromMethod, 
  type LockerTerminal 
} from '@/data/lockerTerminals';
...
export function LockerSearch({ shippingMethod, selectedLocker, onSelect }: LockerSearchProps) {
  ...
  useEffect(() => {
    if (!carrier || query.length < 2) {
      setResults([]);
      return;
    }
    const terminals = searchTerminals(query, carrier);
    setResults(terminals);
    setIsOpen(terminals.length > 0);
  }, [query, carrier]);
